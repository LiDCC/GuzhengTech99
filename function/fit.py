...
class Trainer:
    def __init__(self, model, lr, epoch, save_fn, avg, std, validation_interval, save_interval):
        ...
        self.Xavg, self.Xstd = Variable(Xavg.to(device)), Variable(Xstd.to(device))
    ...
    def fit(self, tr_loader, va_loader, we):
        ...
        viz = Visdom()
        viz.line([[0., 0.]], [0], win='IPT_loss', opts=dict(title='IPT_loss', legend=['train_loss', 'valid_loss']))
        viz.line([[0.]], [0], win='IPT_precision', opts=dict(title='IPT_precision', legend=['valid_IPT_precision']))
        viz.line([[0.]], [0], win='IPT_recall', opts=dict(title='IPT_recall', legend=['valid_IPT_recall']))
        viz.line([[0.]], [0], win='IPT_F1', opts=dict(title='IPT_F1', legend=['valid_IPT_F1']))
        best_acc = 0
        ...
        for e in range(1, self.epoch+1):
            ...
            if e % self.validation_interval == 1:
                ...
                viz.line([[float(loss_total/len(tr_loader.dataset)), float(eva_result[0])]], [e - 1], win='IPT_loss', update='append')
                viz.line([[float(eva_result[1])]], [e - 1], win='IPT_precision', update='append')
                viz.line([[float(eva_result[2])]], [e - 1], win='IPT_recall', update='append')
                viz.line([[float(eva_result[3])]], [e - 1], win='IPT_F1', update='append')
