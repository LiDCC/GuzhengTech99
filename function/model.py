...
class self_att(nn.Module):
    def __init__(self, embeded_dim, num_heads, out):
        ...
        self.att = nn.MultiheadAttention(
            embeded_dim, num_heads, batch_first=True)
...
class SY_multi_scale_attn222(nn.Module):
    def __init__(self):
        ...
        self.us121 = nn.ConvTranspose2d(
            FRE*2, FRE*2, kernel_size=(3, 1), stride=(3, 1))
        ...
        self.us221 = nn.ConvTranspose2d(
            FRE * 3, FRE * 3, kernel_size=(3, 1), stride=(3, 1))
    ...
    def forward(self, x):  # , Xavg, Xstd):
        ...
        x22_n = torch.cat([x22, self.dropout22(self.maxpool212(
            x21)), self.us232(x23, output_size=x22.shape)], -1)
        ...
        x31_n = torch.cat(
            [x31, self.us321(x32, output_size=x31.shape)], dim=-1)
        x32_n = torch.cat([x32, self.us332(
            x33, output_size=x32.shape), self.dropout312(self.maxpool312(x31))], dim=-1)

        x41 = self.block41(self.conv41(x31_n))
        x42 = self.block42(self.conv42(x32_n))

        x51_n = torch.cat(
            [x41, self.us421(x42, output_size=x41.shape)], dim=-1)
